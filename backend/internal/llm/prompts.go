...
Respond ONLY with JSON: {"exerciseName": string, "equipment": string|null, "equipmentToken": string|null, "weightKg": number|null, "reps": number|null, "sets": number|null, "muscles": string[], "clarifyingQuestion": {"token": string, "question": string, "alternatives": string[]}|null}`, line, joinComma(unresolved), shorthand.PromptList())
}
